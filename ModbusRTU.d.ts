...
export class ModbusRTU {
  ...
  writeFC1(
    ...
    dataAddress: number,
    length: number,
    next: NodeStyleCallback<ReadCoilResult>
  ): void;
  writeFC2(
    address: number,
    dataAddress: number,
  ...
  writeFC3(
    ...
    dataAddress: number,
    length: number,
    next: NodeStyleCallback<ReadRegisterResult>
  ): void;
  writeFC4(
    address: number,
    dataAddress: number,
  ...
  writeFC5(
    ...
    dataAddress: number,
    state: boolean,
    next: NodeStyleCallback<WriteCoilResult>
  ): void;
  writeFC6(
    address: number,
    dataAddress: number,
    value: number,
    next: NodeStyleCallback<WriteRegisterResult>
  ): void;
  ...
  writeFC22(
    ...
    andMask: number,
    orMask: number,
    next: NodeStyleCallback<WriteMaskRegisterResult>
  ): void;
  ...
  connectTcpRTUBuffered(
    ip: string,
    options: TcpRTUPortOptions,
    next: Function
  ): void;
  ...
  /**
   * Set DE/RE transmit control GPIO hook
   * @param hooks - callbacks to toggle transmit/receive pin
   */
  setTransmitControl(hooks: {
  ...
  writeRegisters(
    dataAddress: number,
    values: Array<number> | Buffer
  ): Promise<WriteMultipleResult>; // 16
  maskWriteRegister(
    dataAddress: number,
    andMask: number,
    orMask: number
  ): Promise<WriteMaskRegisterResult>;
  ...
  readDeviceIdentification(
    deviceIdCode: number,
    objectId: number
  ): Promise<ReadDeviceIdentificationResult>;
